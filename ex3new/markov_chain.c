...
Node* get_node_from_database(MarkovChain *markov_chain, void *data_ptr){
    ...
    if(curr_node!=NULL) {
        for (; curr_node != NULL; curr_node = curr_node->next) {
            if (markov_chain->comp_func(curr_node->data->data,data_ptr) == 0) {
                return curr_node;
            }
        }
    }
...
Node* add_to_database(MarkovChain *markov_chain, void *data_ptr) {
    ...
    new_markov_ptr->data = markov_chain->copy_func(data_ptr);
...
void free_database(MarkovChain **ptr_chain) {
    ...
    while (curr_node != NULL) {
        ...
        if (curr_node->data->data != NULL) {
            chain->free_data(curr_node->data->data);
        }
        free(curr_node->data);
        Node *temp = curr_node;
...
(MarkovChain *markov_chain, MarkovNode* first_node, int max_length){
    ...
    while (curr_node && words_counter < max_length) {
        ...
        words_counter++;
        if (curr_node->is_end_of_sentence
