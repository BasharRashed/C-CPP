...
typedef struct MarkovNode{
    ...
    int freq_list_size;
    /**
     * @param is_end_of_sentence is 1 when it has a dot.
     * and 0 if it doesn't.
 */
    int is_end_of_sentence;
 ...
 * If so, return the Node wrapping it in
 * the markov_chain, otherwise return NULL.
 * @param markov_chain the chain
 * to look in its database
 * @param data_ptr the data to look for
 * @return Pointer to the Node
 * wrapping given data, NULL if state not in
 * database.
 */
Node* get_node_from_database(MarkovChain *markov_chain, char *data_ptr);

/**
* If data_ptr in markov_chain,
 * return it's node. Otherwise, create new
 * node, add to end of
 * markov_chain's database and return it.
 * @param markov_chain the chain
 * to look in its database
 * @param data_ptr the data to look for
 * @return Node wrapping given data_ptr
 * in given chain's database,
...
/**
 * Add the second markov_node to the
 * frequency list of the first markov_node.
 * If already in list, update it's
 * occurrence frequency value.
 * @param first_node
 * @param second_node
 * @return success/failure: 0 if the
 * process was successful, 1 if in
...
/**
 * Free markov_chain and all
 * of it's content from memory
 * @param markov_chain markov_chain to free
 */
void free_database(MarkovChain ** ptr_chain);

/**
 * Get one random MarkovNode from
 * the given markov_chain's database.
 * @param markov_chain
...
/**
 * Choose randomly the next MarkovNode,
